...
function createPainfulOreSmeltingRecipes(event, material) {
    let oreProperty = material.getProperty(PropertyKey.ORE);

    if (oreProperty == null && material.hasProperty(PropertyKey.INGOT)) return;

    let directSmeltMaterial = oreProperty.getDirectSmeltResult();

    if (directSmeltMaterial == null) return;

    let materialComponents = material.getMaterialComponents();

    let materialComponentsTotal = 0;

    let outputCount = 1;

    for (const component of materialComponents) {
        materialComponentsTotal += component.amount();

        if (component.material() == directSmeltMaterial) {
            outputCount = component.amount();
        }
    }

    processOreSmeltingRecipes(event, material, materialComponentsTotal, outputCount)
}


function processOreSmeltingRecipes(event, material, inputCount, outputCount) {
    const materialName = material.getName();
    ...
    const outputMaterial = oreProperty.getDirectSmeltResult();

    if (outputMaterial == null || outputMaterial.getProperty(PropertyKey.INGOT) == null) return;

    const ingotItem = ChemicalHelper.get(TagPrefix.ingot, outputMaterial, outputCount);

    const poorRawOreItem = ChemicalHelper.get(InfinityTagPrefix.poorRawOre, material, inputCount);
    const rawOreItem = ChemicalHelper.get(TagPrefix.rawOre, material, inputCount);
    const richRawOreItem = ChemicalHelper.get(InfinityTagPrefix.richRawOre, material, inputCount);

    const crushedOreItem = ChemicalHelper.get(TagPrefix.crushed, material, inputCount);
    const purifiedOreItem = ChemicalHelper.get(TagPrefix.crushedPurified, material, inputCount);
    const refinedOreItem = ChemicalHelper.get(TagPrefix.crushedRefined, material, inputCount);
    const impureDustItem = ChemicalHelper.get(TagPrefix.dustImpure, material, inputCount);
    const purifiedDustItem = ChemicalHelper.get(TagPrefix.dustPure, material, inputCount);
    const dustItem = ChemicalHelper.get(TagPrefix.dust, material, inputCount);

    event.recipes.gtceu.electric_furnace(`smelt_poor_raw_${materialName}_ore_to_ingot`)
        .itemInputs(poorRawOreItem.withCount(inputCount*2))
        .itemOutputs(ingotItem)
        .duration(200*outputCount)
        .EUt(GTValues.VH[GTValues.ULV]);

    event.recipes.gtceu.electric_furnace(`smelt_raw_${materialName}_ore_to_ingot`)
        .itemInputs(rawOreItem)
        .itemOutputs(ingotItem)
        .duration(200*outputCount)
        .EUt(GTValues.VH[GTValues.ULV]);

    event.recipes.gtceu.electric_furnace(`smelt_rich_raw_${materialName}_ore_to_ingot`)
        .itemInputs(richRawOreItem)
        .itemOutputs(ingotItem.withCount(outputCount*2))
        .duration(200*(outputCount*2))
        .EUt(GTValues.VH[GTValues.ULV]);

    event.recipes.gtceu.electric_furnace(`smelt_crushed_${materialName}_ore_to_ingot`)
        .itemInputs(crushedOreItem)
        .itemOutputs(ingotItem)
        .duration(200*outputCount)
        .EUt(GTValues.VH[GTValues.ULV]);

    event.recipes.gtceu.electric_furnace(`smelt_purified_${materialName}_ore_to_ingot`)
        .itemInputs(purifiedOreItem)
        .itemOutputs(ingotItem)
        .duration(200*outputCount)
        .EUt(GTValues.VH[GTValues.ULV]);

    event.recipes.gtceu.electric_furnace(`smelt_refined_${materialName}_ore_to_ingot`)
        .itemInputs(refinedOreItem)
        .itemOutputs(ingotItem)
        .duration(200*outputCount)
        .EUt(GTValues.VH[GTValues.ULV]);

    event.recipes.gtceu.electric_furnace(`smelt_impure_${materialName}_dust_to_ingot`)
        .itemInputs(impureDustItem)
        .itemOutputs(ingotItem)
        .duration(200*outputCount)
        .EUt(GTValues.VH[GTValues.ULV]);

    event.recipes.gtceu.electric_furnace(`smelt_purified_${materialName}_dust_to_ingot`)
        .itemInputs(purifiedDustItem)
        .itemOutputs(ingotItem)
        .duration(200*outputCount)
        .EUt(GTValues.VH[GTValues.ULV]);

    event.recipes.gtceu.electric_furnace(`smelt_${materialName}_dust_to_ingot`)
        .itemInputs(dustItem)
        .itemOutputs(ingotItem)
        .duration(200*outputCount)
        .EUt(GTValues.VH[GTValues.ULV]);
